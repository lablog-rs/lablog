...
impl Options {
    pub fn try_from(matches: &ArgMatches) -> Result<Self> {
        ...
        let source_dir: PathBuf = {
            let arg = matches.value_of("source_dir").chain_err(
                || "can not get source_dir from args",
            )?;
        ...
        let destination_dir: PathBuf = {
            let arg = matches.value_of("destination_dir").chain_err(
                || "can not get destination_dir from args",
            )?;
