    ...
    pub fn try_from(matches: &ArgMatches) -> Result<Self> {
        let datadir = match matches.value_of("datadir").unwrap() {
            "$XDG_DATA_HOME/lablog" => {
                let xdg = BaseDirectories::new().chain_err(|| "can not get xdg base directory")?;

                xdg.create_data_directory("lablog")
                    .chain_err(|| "can not create xdg base directory")?
            }
            _ => PathBuf::from(matches
                .value_of("datadir")
