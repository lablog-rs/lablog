...
pub fn git_commit_note(datadir: &PathBuf, project: Project, note: Option<&Note>, message: &str) {
    ...
    githelper::add(datadir, Path::new(project_path.as_str())).expect("can not add project file changes to git");
...
pub fn write_project(datadir: &PathBuf, project: Project, notes: &Notes, overwrite: bool) -> Option<()> {
...
pub fn filter_notes_by_timestamp(notes: ProjectsNotes, timestamp: DateTime<UTC>, after: bool) -> ProjectsNotes {
...
fn test_get_parent() {
    ...
    assert_eq!(
        Some(String::from("athaller")),
        get_parent(Some(String::from("athaller.test")))
    );

    assert_eq!(
        Some(String::from("athaller.test")),
...
pub fn archive_project(datadir: &PathBuf, projects: Projects, project: Project, recursive: bool) {
    ...
    githelper::add(datadir, Path::new(new_path.as_path())).expect("can not add project archive file to git");

    githelper::add(datadir, Path::new(old_path.as_path())).expect("can not add removed project file to git");

    let commit_message = format!("{} - {} - archived", Local::now(), project.unwrap());

    githelper::commit(datadir, commit_message.as_str()).expect("can not commit archive move to repo");
}
